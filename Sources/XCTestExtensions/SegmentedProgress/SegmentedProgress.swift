...
///
/// `maxValue` must be greater than `minValue`. `progress(value: T)` will not produce output if `value` is less than `minValue`, however
/// `value` > `maxValue` will continue producing outputs > 100%.
///
